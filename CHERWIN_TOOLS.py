...
def CHECK():
    ...
    # 本地JSON文件路径
    local_json_path = os.path.join('/arcadia/raw/CHERWIN_SCRIPT_CONFIG.json')  # 替换为你的本地路径
    TOOLS_NAME = 'CHERWIN_TOOLS.py'
    server_script_url = f'https://github.com/a001980/CHERWING/blob/main/{TOOLS_NAME}'
    
    try:
        print(f'读取本地文件: {local_json_path}')
        ...
        print(f'获取CHERWIN_SCRIPT_CONFIG.json失败: {e}')
        return False
...
def main(APP_NAME, local_script_name, ENV_NAME, local_version, need_invite=False):
    ...
    git_url = f'https://github.com/a001980/CHERWING/blob/main/{local_script_name}'
    if CHECK():
